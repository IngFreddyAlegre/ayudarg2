...
class EavBehavior extends ModelBehavior {
     ...
     * @var unknown_type
     */
    var $dataTypeFieldName = 'data_type_id';
    /**
     * 
    ...
    function addAttributeValue($attribute, $value, $entityId = null) {
        ...
        $alias = $this->valueModels[$attribute['Attribute']['data_type_id']];
        ...
        if ($model != null) {
            ...
            if (! empty($fields)) {
                foreach ($fields as $field) {
                    $dataTypeName = $this->Attribute->DataType->field('name',array('id' => $field['Attribute']['data_type_id']));
                    $valueModel = $this->valueModels[$dataTypeName];
                    $this->$valueModel = ClassRegistry::init($valueModel);
                    $dbo = $this->$valueModel->getDataSource();
                    $table = 'attributes_' . $dataTypeName . '_values';
        ...
        if ($entityId == null) {
            ...
            foreach ($this->valueModels as $dataType => $dataModel) {
                ...
                foreach ($values as $key => $value) {
                    //TODO
                    $result['AttributeValue'][$value[$this->attributeModel]['name']] = $value[$dataModel]['value'];
                }
            }
            return result;
        }
    ...
    function afterSave(Model $model, $created, $options = array()) {
        ...
        foreach ($data[$model->name] as $field => $value) {
            if ($model->isVirtualField($field) || $this->getAttributeIdByName($field)) {
                ...
                $dataTypeName = $this->Attribute->DataType->field('name',array('id' => $attribute[$this->attributeModel]['data_type_id']));
                $dataModel = $this->valueModels[$dataTypeName];
